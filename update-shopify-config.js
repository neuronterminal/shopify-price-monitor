...
export async function action({ request }) {
  ...
  const headers = {
    "Access-Control-Allow-Origin": "https://b46a-71-183-35-90.ngrok-free.app",
  ...
  try {
    ...
    try {
      ...
      console.log("Auth result:", authResult ? "exists" : "undefined");
      
      if (!authResult || !authResult.admin) {
        console.error("Authentication failed or returned unexpected result:", authResult);
        return json(
          { error: "Authentication failed. Make sure you're logged into Shopify." }, 
          { status: 401, headers }
        );
      }
    ...
    try {
      ...
      if (productsData.errors) {
        ...
        return json(
          { error: "GraphQL error: " + productsData.errors[0]?.message }, 
          { status: 500, headers }
        );
      }
    } catch (graphqlError) {
      ...
      return json(
        { error: "Failed to fetch products: " + graphqlError.message }, 
        { status: 500, headers }
      );
    }
    ...
    try {
      ...
      return json(
        { error: "Some updates failed: " + updateError.message }, 
        { status: 500, headers }
      );
    }
  } catch (error) {
